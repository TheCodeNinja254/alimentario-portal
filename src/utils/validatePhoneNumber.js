...
const validatePhoneNumber = (format = 'all', full = false, text) => {
  ...
  const validationMap = {
    'leading_zero': {
      ...
      format: full ? /0[71]\d{8}/g : (text.length > 2 ? /0[71]\d{1,8}/g : /0[71]/g),
    ...
    'full': {
      // eslint-disable-next-line no-nested-ternary
      format: full ? /254[71]\d{8}/g : (text.length > 3 ? /254[71]\d{0,8}/g : /\d*/),
      error: 'Phone number should match the pattern 254XXXXXXXXX',
      maxLength: 12
    },
    all: {
      // eslint-disable-next-line no-nested-ternary
      format: full ? /(([71])|(0[71])|(\+254[71])|(254[71]))\d{8}/g : (text.length > 3 ? /([71]\d{0,8})|(254[71]\d{0,8})|(\+254[71]\d{0,8})|(0[71]\d{0,8})/g : (/\d*/)),
  ...
  const isValid = matcher != null && matcher.length > 0 && matcher.shift() === text;
  const result = {
    isValid,
    validationError: '',
  ...
  if (!isValid) {
    result.validationError = text.length > validationMap[format].maxLength ?
